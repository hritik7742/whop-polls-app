-- Complete database setup for real-time polls application
...
-- You may need to adjust them based on your specific authentication setup
...
-- Create a scheduled job to update poll status (optional)
-- This requires the pg_cron extension to be enabled
-- SELECT cron.schedule('update-poll-status', '*/5 * * * *', 'SELECT update_poll_status();');
