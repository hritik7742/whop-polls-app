...
export function useRealtimeActivePolls(experienceId: string, userId: string) {
  ...
  const processPollsData = useCallback((pollsData: any[], optionsData: any[], votesData: any[]) => {
    ...
    // Create polls map - ONLY for active polls
  ...
  const fetchPolls = useCallback(async () => {
    ...
    try {
      ...
      // Get only ACTIVE polls for experience view
      // Check for polls that match experience_id OR have company_id as experience_id (for dashboard-created polls)
      const { data: pollsData, error: pollsError } = await supabase
        .from('polls')
        .select('*')
        .or(`experience_id.eq.${experienceId},company_id.eq.${experienceId}`)
      ...
      // Debug logging
      console.log('Active polls query for experience:', experienceId);
      console.log('Found polls:', pollsData?.length || 0);
      if (pollsData && pollsData.length > 0) {
        console.log('Poll details:', pollsData.map(p => ({
          id: p.id,
  ...
  }, [experienceId, processPollsData]);

  // Optimized real-time update handler
  const handleRealtimeUpdate = useCallback((payload: any) => {
    console.log('Real-time update received for active polls:', payload);
  ...
  useEffect(() => {
    ...
    tables.forEach(table => {
      const channelName = `active_${table}_changes_${experienceId}_${userId}`;
      const subscription = supabase!
        ...
        .on(
          'postgres_changes',
          {
            ...
            schema: 'public',
            table: table,
            // No filter for polls - we'll handle filtering in the fetch function
          },
          (payload) => {
            console.log(`Real-time update from ${table} for active polls:`, payload);
            handleRealtimeUpdate(payload);
          }
        )
        .subscribe((status) => {
          console.log(`Active polls subscription status for ${table}:`, status);
          if (status === 'SUBSCRIBED') {
            console.log(`✅ Subscribed to ${table} changes for experience ${experienceId}`);
  ...
  }, [experienceId, fetchPolls, handleRealtimeUpdate]);
