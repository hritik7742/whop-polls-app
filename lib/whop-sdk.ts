...
export function getWhopSdk() {
	if (!whopSdkInstance) {
		...
		whopSdkInstance = WhopServerSdk({
			...
			// Don't set onBehalfOfUserId here - let verifyUserToken handle user identification
			// The onBehalfOfUserId should only be used for agent operations, not user operations
			// onBehalfOfUserId: process.env.NEXT_PUBLIC_WHOP_AGENT_USER_ID,

			// Don't set companyId here - it should be determined per request
